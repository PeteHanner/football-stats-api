...
RSpec.describe SecondOrderGameStatsWorker, type: :worker do
  describe "#perform" do
    ...
    it "raises error if opponent not found" do
      ...
      error_msg = "#{described_class.name} unable to set opponent of team ID #{team.id} on game ID #{game.id}"
    ...
    it "raises error if unable to save stats" do
      ...
      error_msg = "#{described_class.name} encountered error processing stats for teams #{team.id} & #{opponent.id} on game #{game.id}: ERROR MESSAGE"
